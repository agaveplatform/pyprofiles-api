...
fig -f docker-compose.multitenant.yml up -d
...
curl -X POST --data "username=bbking&password=abcde&email=bbking@music.org" localhost:8000/vdj-org/profiles/v2
curl -X POST --data "username=rasputin&password=abcde&email=rasputin@aip.org" localhost:8000/araport-org/profiles/v2
